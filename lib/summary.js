...
function _ipChecksRate(checks, ip) {
  const ipChecks = checks.filter((c) => c.path.includes(`IP de Origen: ${ip}`));
...
function ipEndpointCheckStats(checks, ip, ep) {
  const relevant = checks.filter((c) => {
    if (!c.path.includes(`IP de Origen: ${ip}`)) return false;
    const haystack = `${c.path} ${c.name}`.toLowerCase();
    return haystack.includes(ep.title.toLowerCase()) || haystack.includes(ep.tag.replace('_', '/'));
  });
...
function buildIpBusinessDistribution(endpoints, checks, ip) {
  const rows = endpoints
    .map((ep) => {
      const stats = ipEndpointCheckStats(checks, ip, ep);
      return `<tr>
      ...
      <td style="background:#ffebee;color:#b71c1c;font-weight:700">${stats.unexpected}</td>
    </tr>`;
    })
    .join('');
...
function buildAuditMatrix(ips, durationStr) {
  const rows = ips
    .map((ip, index) => {
      ...
      return `<tr>
      <td class="left strong mono">${escapeHtml(ip.ip)}</td>
      <td>Nodo ${index + 1}</td>
      <td>${ip.reqs}</td>
      <td>${fmtMs(ip.avg)}</td>
  ...
  return `<details class="section" open>
    <summary>🖥️ Matriz de Auditoria — Desglose de métricas por IP (${ips.length} IPs del pool)
...
function buildAuditRecommendations(p95, errorRate, checksRate, apdex, SLO) {
  ...
  const allPass = p95Pass && errorPass && checksPass && apdexPass;
  ...
  if (!p95Pass) {
    recommendations.push(
      `<li><strong>Latencia P95 fuera de SLO (${fmtMs(p95)} vs &lt; ${SLO.p95}ms):</strong> Se detectó saturación o encolamiento en el backend. Revisar performance de base de datos e índices en consultas concurrentes.</li>`,
    );
  }
  if (!errorPass) {
    recommendations.push(
      `<li><strong>Tasa de error elevada (${fmtPct(errorRate)} vs &lt; ${fmtPct(SLO.errorRate)}):</strong> La tasa de fallas excede el límite tolerable de 1%. Esto suele ser causado por políticas agresivas de Rate Limit (HTTP 429) o fallas en el servidor (HTTP 5xx). Evaluar incremento de umbrales de rate limit en API Gateway.</li>`,
    );
  }
  if (!checksPass) {
    recommendations.push(
      `<li><strong>Checks funcionales fallidos (${fmtPct(checksRate)} vs &gt; ${fmtPct(SLO.checks)}):</strong> Errores en la lógica de negocio o respuestas inesperadas del backend. Validar la consistencia de datos de prueba y estabilidad del servicio.</li>`,
    );
  }
  if (!apdexPass) {
    recommendations.push(
      `<li><strong>Índice APDEX degradado (${fmtFixed(apdex, 3)} vs &gt; ${fmtFixed(SLO.apdex, 2)}):</strong> La experiencia percibida por los usuarios es subóptima debido a latencias elevadas en una parte considerable de las transacciones.</li>`,
    );
  }
  ...
  return `<details class="section" open>
    <summary>📋 Recomendaciones Técnicas y Criterios de Salida QA (ISTQB / SRE)
    ...
    .map((ip, index) => {
      ...
      <div style="padding: 14px;">
        ...
        <div class="sub-card">
          ...
          ${buildIpBusinessDistribution(endpoints, checks, ip.ip)}
        </div>
      </div>
    </details>`;
    })
  ...
  return endpoints
    .map((ep) => {
      ...
      const unexpected =
        stats.unexpected ||
        (endpoints.length === 1
          ? [400, 401, 403, 404, 500, 502, 503, 504].reduce(
              (sum, st) => sum + countMetric(data, `http_reqs{status:${st}}`),
              0,
            )
          : 0);
...
function buildHtml(testName, opts, data) {
  ...
  const isAudit = scenarioName(testName) === 'multi_ip_audit';
  ...
  const globalErrorRate = fail ? num(fail.rate ?? fail.value, 0) : 0;
...
<body>
  <div class="page">
    ...
    ${isAudit ? buildAuditMatrix(ips, durationStr) : ''}
    ${isAudit ? buildAuditRecommendations(globalP95, globalErrorRate, globalChecksRate, globalApdex, SLO) : ''}
