...
import { businessLimitHits, sessionSuccessRate, httpOutcomeCounter } from './metrics.js';
...
export function classifyAndCheck(res, requestName, ip = '') {
  let body = {};
  try { body = res.json(); } catch (e) { body = { bSuccess: false, sMessage: res.body }; }

  const status   = res.status;
  const msg      = (body && body.sMessage) || '';
  const iter     = __ITER + 1;
  const vuNum    = __VU;
  ...
  group(groupLabelIp, function () {
    group(groupLabelIter, function () {
      if (status === 200 && body.bSuccess === true) {
        ...
        check(res, { [`${ipPrefix}[HTTP 200] Success: false - Límite ${tipo} | Msj: ${msg.substring(0,100)}`]: () => true });
      } else if (status === 429) {
        outcome = 'gateway_429';
