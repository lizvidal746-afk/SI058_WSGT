...
// =====================
const carnetDocs = new SharedArray('docs_carnet', () =>
  parseCsvWithHeader(open('../data/documentos_carnet.csv'), ['numeroDocumento']),
);
const gradosDocs = new SharedArray('docs_grados', () =>
  parseCsvWithHeader(open('../data/documentos_grados.csv'), ['numeroDocumento']),
);
// =====================
// USO RECOMENDADO:
...
export function getCarnetUser() {
  const isMulti = __VU > 1 || __ENV.SCENARIO?.includes('multi');
...
export function getResetUser() {
  return pickByVU(resetUsers) || resetUsers[0] || null;
}
