...
async function crearPlanPruebas() {
  ...
  const casos = [
    caso(1, 'CP-CAR-00', 'Carné', 'Smoke Baseline (Single IP)', 'Smoke', 'npm run perf:carnet:smoke',
    ...
    caso(2, 'CP-CAR-01', 'Carné', 'Auditoría Forense (Multi-IP)', 'Audit (Multi-IP)', 'npm run perf:audit',
      '10 IPs (.48-.57) / Usuarios 2-11 / CSV Carnet.',
      'Latencia p95 < 1.5s. Trazabilidad total de las 10 IPs en DB sin pérdida.'),
    caso(3, 'CP-CAR-02', 'Carné', 'Carga Base Orgánica (Multi-IP)', 'Baseline / Load', 'npm run perf:cp02',
      '11 IPs / 11 VUs / 10 iteraciones por VU.',
      'Línea base estable: p95 < 1.5s, p99 controlado, APDEX >= 0.90, 0 errores 5xx sostenidos.'),
    caso(4, 'CP-CAR-03', 'Carné', 'Escalabilidad Multi-PC', 'Stress (Ramping)', 'npm run perf:stress',
    ...
    caso(5, 'CP-CAR-04', 'Carné', 'Límite de Red WAF (Single IP)', 'WAF Limit', 'npm run perf:cp01',
    ...
    caso(6, 'CP-CAR-05', 'Carné', 'Saturación Backend (Extreme)', 'Breakpoint / Stress Extreme', 'npm run perf:cp03',
    ...
    caso(7, 'CP-CAR-06', 'Carné', 'Colapso Controlado', 'Collapse / Breakpoint Rápido', 'npm run perf:collapse',
