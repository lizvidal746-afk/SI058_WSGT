...
const {
  ...
  PageNumber, Footer, BorderStyle, VerticalAlign, ImageRun,
} = require('docx');
...
// ── Paleta SUNEDU ────────────────────────────────────────────────────────────
const C = {
  AZU: '1A237E', AZM: '3949AB', AZC: 'E8EAF6',
  BLA: 'FFFFFF', GRS: 'F5F5F5',
  VEF: 'E8F5E9', ROF: 'FFEBEE', AMF: 'FFF8E1',
  VER: '2E7D32', ROJ: 'C62828', AMA: 'F9A825',
};

// ── Helpers de construcción docx ─────────────────────────────────────────────
const txt = (t, o = {}) => new TextRun({
  text: String(t ?? ''), bold: o.bold, italic: o.italic,
  color: o.color ?? '111111', size: (o.size ?? 9.5) * 2, font: 'Calibri',
});
...
const table = (rows, widthPct = 100) => new Table({
  width: { size: widthPct, type: WidthType.PERCENTAGE },
  rows,
});

const h2 = (text) => para(
  [txt(text, { bold: true, size: 11, color: C.AZM })],
  { sb: 320, sa: 80 }
);
...
const LEGEND = [
  ...
  ['p(95) ★ SLO Principal',    'El 95% de usuarios recibe respuesta en ≤ este tiempo.',     '< 1 500 ms', 'Google SRE · ISO 25023'],
    ...
    sections: [{
      ...
      children: [
        ...
        para([txt('INFORME TÉCNICO DE RENDIMIENTO', { bold: true, size: 16, color: C.AZU })],
          { align: AlignmentType.CENTER, sb: 400 }),
        para([txt('AUDITORÍA TDR — SUNEDU', { bold: true, size: 13, color: C.AZM })],
          { align: AlignmentType.CENTER, sa: 400 }),

        table([
          row(['Entidad',              'SUNEDU — Superintendencia Nacional de Educación Superior Universitaria'], { bg: C.AZC, bold: true, ws: [30, 70] }),
          ...
          row(['Estándares Aplicados',  'ISTQB PT · ISO/IEC 25010 · ISO/IEC 25023 · Google SRE'],                 { bg: C.AZC, ws: [30, 70] }),
          row(['Estado Final',          `${r.statusEmoji} ${r.status}`],
            { bold: true, bg: r.sloPass ? C.VEF : C.ROF, color: r.sloPass ? C.VER : C.ROJ, ws: [30, 70] }),
        ]),

        para([txt('CONFIDENCIAL — PROPIEDAD DE SUNEDU', { italic: true, size: 8, color: '757575' })],
          { align: AlignmentType.CENTER, sb: 200 }),
        ...
        table([
          ...
          row(['Latencia p95 ★ SLO',    fmtMs(r.p95),                            '< 1 500 ms',  p95ok   ? '✅ OK' : '🔴 FALLA'], { bold: true, bg: statusBg(p95ok),   color: statusColor(p95ok)  }),
          row(['Latencia p99 (Cola)',    fmtMs(r.p99),                            '< 2 000 ms',  p99ok   ? '✅ OK' : '🔴 FALLA'], { bg: statusBg(p99ok),   color: statusColor(p99ok)  }),
          row(['Tasa de Errores',        fmtPct(r.errorRate),                     '< 1%',        errOk   ? '✅ OK' : '🔴 FALLA'], { bold: true, bg: statusBg(errOk),   color: statusColor(errOk)  }),
          row(['APDEX Score',            r.apdex.toFixed(3),                      '≥ 0.90',      apdexOk ? '✅ OK' : '🔴 FALLA'], { bg: statusBg(apdexOk), color: statusColor(apdexOk) }),
          row(['Checks OK',             fmtPct(r.checksRate),                    '> 99%',       chkOk   ? '✅ OK' : '🔴 FALLA'], { bg: statusBg(chkOk),   color: statusColor(chkOk)  }),
        ...
        table([
          row(['Fase de Latencia', 'Promedio (avg)', 'p95', 'Descripción', 'Umbral Ref.'], { header: true, ws: [25, 15, 15, 35, 10] }),
          row(['TTFB (http_req_waiting)',        fmtMs(r.ttfb),        fmtMs(r.ttfbP95),    'Tiempo puro de procesamiento del backend/BD',     '—'],        { ws: [25, 15, 15, 35, 10] }),
          row(['Bloqueo TCP (http_req_blocked)', fmtMs(r.blockedAvg),  fmtMs(r.blockedP95), 'Espera de socket libre. Alto = saturación de red', '< 10 ms'], { bg: C.GRS, ws: [25, 15, 15, 35, 10] }),
          row(['TLS Handshake',                  fmtMs(r.tlsAvg),      fmtMs(r.tlsP95),     'Negociación SSL/TLS. Alto = problema certificado', '< 50 ms'], { ws: [25, 15, 15, 35, 10] }),
          row(['Conexión TCP',                   fmtMs(r.connectingAvg),'—',                'Apertura de conexión TCP',                        'Red'],     { bg: C.GRS, ws: [25, 15, 15, 35, 10] }),
          row(['Envío (sending)',                fmtMs(r.sendingAvg),  '—',                 'Tiempo enviando el request HTTP',                 '—'],        { ws: [25, 15, 15, 35, 10] }),
          row(['Recepción (receiving)',           fmtMs(r.receivingAvg),'—',                'Tiempo recibiendo la respuesta del servidor',      '—'],        { bg: C.GRS, ws: [25, 15, 15, 35, 10] }),
          row(['Duración de Iteración',           fmtMs(r.iterationAvg),'—',               'Ciclo completo de VU (setup + request + teardown)','—'],        { ws: [25, 15, 15, 35, 10] }),
        ]),
        ...
        table([
          ...
          row(['Error Budget consumido',`${budget.consumedPct}%`],           { bold: true, bg: budget.consumedPct > 80 ? C.ROF : C.VEF, color: budget.consumedPct > 80 ? C.ROJ : C.VER }),
        ...
        ...(epSummary.length > 0 ? [
          ...
          table([
            row(['Servicio', 'Reqs', 'avg', 'p50', 'p95 ★', 'p99', 'max', 'Error %', 'Checks'], { header: true, ws: [20, 10, 10, 10, 10, 10, 10, 10, 10] }),
            ...epSummary.map(ep => {
          ...
          para([txt('⚠️ No se encontraron grupos de endpoint en root_group. Verifique que el script k6 usa group() con nombres de endpoint.', { color: C.ROJ })]),
        ]),
        ...
        ...(epIters.length > 0 ? [
          ...
          table([
            row(['Iteración', 'Servicio', 'URL', 'Checks OK', 'Checks Fail'], { header: true, ws: [20, 15, 40, 12, 13] }),
            ...epIters.map(iter => row([
        ...
        ...(() => {
          ...
          Object.keys(r._m).forEach(key => {
            ...
            if (match) {
              ...
              if (count > 0) {
                let type = 'Desconocido', color = C.GRS, textC = '757575', emoji = '❔';
                if (status >= 200 && status < 300) { type = 'Éxito (OK)'; color = C.VEF; textC = C.VER; emoji = '✅'; }
                else if (status >= 300 && status < 400) { type = 'Redirección'; color = C.AZC; textC = C.AZM; emoji = '↪️'; }
                else if (status >= 400 && status < 500) { type = 'Error de Cliente'; color = C.AMF; textC = 'E65100'; emoji = '⚠️'; }
                else if (status >= 500) { type = 'Error de Servidor'; color = C.ROF; textC = C.ROJ; emoji = '🔴'; }
                if (status === 429) { type = 'Too Many Requests (Rate Limit)'; emoji = '⏸️'; }
          ...
          return [
            ...
            para([txt('A continuación se detallan los códigos de estado HTTP emitidos por el servidor. Un alto volumen de errores 4xx o 5xx indica que el sistema entró en estado de saturación o colapso bajo la carga.')]),
            table([
              row(['Código HTTP', 'Clasificación', 'Frecuencia'], { header: true, ws: [25, 50, 25] }),
              ...statusCounts.map((st, idx) => row([
                `${st.emoji} HTTP ${st.status}`,
                st.type,
                String(st.count)
              ], {
                bg: st.color,
        ...
        ...(r.localIps.length > 1 ? [
          ...
          table([
            row(['IP Origen (Cliente PC)', 'Reqs', 'avg', 'p50', 'p95 ★ SLO', 'p99', 'Error %', 'TTFB', 'Tiempo Prueba'], { header: true, ws: [20, 8, 8, 8, 12, 8, 10, 11, 15] }),
            ...r.localIps.map(ip => {
        ...
        table([
          ...
          row(['Checks fallidos',         String(r.checksFails),                  '0',    r.checksFails === 0 ? '✅ OK' : '🔴 FALLA'], { bg: r.checksFails === 0 ? C.GRS : C.ROF }),
          row(['Tasa de éxito (%)',        fmtPct(r.checksRate),                  '> 99%', chkOk ? '✅ OK' : '🔴 FALLA'], { bold: true, bg: statusBg(chkOk), color: statusColor(chkOk) }),
          row(['Session Success Rate',     fmtPct(r.sessionRate),                 '> 99%', sessOk ? '✅ OK' : '🔴 FALLA'], { bg: statusBg(sessOk), color: statusColor(sessOk) }),
        ]),
        ...
        table([
          ...
          row(['Latencia p95 < 1 500 ms',    '< 1 500 ms', fmtMs(r.p95),            p95ok   ? '✅ PASA' : '🔴 FALLA'], { bg: statusBg(p95ok)   }),
          row(['Tasa de errores < 1%',        '< 1%',       fmtPct(r.errorRate),     errOk   ? '✅ PASA' : '🔴 FALLA'], { bg: statusBg(errOk)   }),
          row(['Checks funcionales > 99%',    '> 99%',      fmtPct(r.checksRate),    chkOk   ? '✅ PASA' : '🔴 FALLA'], { bg: statusBg(chkOk)   }),
          row(['APDEX Score ≥ 0.90',          '≥ 0.90',     r.apdex.toFixed(3),      apdexOk ? '✅ PASA' : '🔴 FALLA'], { bg: statusBg(apdexOk) }),
          row(['Error Budget < 80% consumido','< 80%',      budget.consumedPct + '%', budget.consumedPct < 80 ? '✅ PASA' : '⚠️ ALERTA'], { bg: budget.consumedPct < 80 ? C.VEF : C.AMF }),
        ]),
        ...
        ...[
          ...
          `• Error Budget consumido: ${budget.consumedPct}%. ` +
            (budget.consumedPct < 50 ? 'Margen amplio para escalar a Load/Stress con confianza.' : 'Optimizar latencia antes de prueba de carga.'),
...
main().catch(err => { console.error('❌ [WORD]', err.message); process.exit(1); });
